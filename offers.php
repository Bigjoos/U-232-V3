...
$posted_action = strip_tags((isset($_GET['action']) ? $_GET['action'] : (isset($_POST['action']) ? $_POST['action'] : '')));
	...
	$valid_actions = array('add_new_offer', 'delete_offer', 'edit_offer', 'offer_details', 'vote', 'add_comment', 'edit_comment', 'delete_comment', 'alter_status');
...
//===========================================================================================//
    ...
    $res_did_they_vote = sql_query('SELECT vote FROM offer_votes WHERE user_id = '.$CURUSER['id'].' AND offer_id = '.$id);
...
if ($row_did_they_vote[0] == '')
    ...
    $yes_or_no = ($vote == 1 ? 'yes' : 'no');
        ...
        sql_query('INSERT INTO offer_votes (offer_id, user_id, vote) VALUES ('.$id.', '.$CURUSER['id'].', \''.$yes_or_no.'\')');
        sql_query('UPDATE offers SET '.($yes_or_no == 'yes' ? 'vote_yes_count = vote_yes_count + 1' : 'vote_no_count = vote_no_count + 1').' WHERE id = '.$id);
...
$HTMLOUT .= '<table border="0" cellspacing="0" cellpadding="5" align="center">
    ...
    while ($main_query_arr = mysqli_fetch_assoc($main_query_res)) 
    {
        ...
        $status = ($main_query_arr['status'] == 'approved' ? '<span style="color: limegreen;font-weight: bold;">approved!</span>' : 
                    ($main_query_arr['status'] == 'pending' ? '<span style="color: skyblue;font-weight: bold;">pending...</span>' : 
 ...
 $HTMLOUT .= '
    <tr>
        ...
        <td class="'.$class.'" align="left"><a class="altlink" href="offers.php?action=offer_details&amp;id='.$main_query_arr['offer_id'].'">'.htmlspecialchars($main_query_arr['offer_name'], ENT_QUOTES).'</a></td>
...
$status_drop_down = ($CURUSER['class'] < UC_STAFF ? '' : '<br /><form method="post" action="offers.php">
 ...
 //=== start page 
 $HTMLOUT .= (isset($_GET['status_changed']) ? '<h1>Offer Status Updated!</h1>' : '' ).(isset($_GET['voted']) ? '<h1>vote added</h1>' : '' ).
        (isset($_GET['comment_deleted']) ? '<h1>comment deleted</h1>' : '' ).$top_menu.
        ($arr['status'] == 'approved' ? '<span style="color: limegreen;font-weight: bold;">status: approved!</span>' : 
                    ($arr['status'] == 'pending' ? '<span style="color: skyblue;font-weight: bold;">status: pending...</span>' : 
    ...
    <tr>
        ...
        ($CURUSER['class'] < UC_STAFF ? '' : ' [ <a href="offers.php?action=edit_offer&amp;id='.$id.'">edit</a> ] 
    ...
    <tr>
        ...
        <td align="left" class="two"><img border="0" src="pic/caticons/'.$CURUSER['categorie_icon'].'/'.htmlspecialchars($arr['cat_image'], ENT_QUOTES).'" alt="'.htmlspecialchars($arr['cat_name'], ENT_QUOTES).'" /></td>
    </tr>
    <tr>
        <td class="two" align="right">link:</td>
        <td align="left" class="two"><a class="altlink" href="'.htmlspecialchars($arr['link'], ENT_QUOTES).'"  target="_blank">'.htmlspecialchars($arr['link'], ENT_QUOTES).'</a></td>
    </tr>
    <tr>
...
$HTMLOUT .= '<h1>Comments for '.htmlentities($arr['offer_name'], ENT_QUOTES ).'</h1>
	...
	list($menu, $LIMIT) = pager_new($count, $perpage, $page, 'offers.php?action=offer_details&amp;id='.$id, ($perpage == 20 ? '' : '&amp;perpage='.$perpage).'#comments');  
		...
		$subres = sql_query('SELECT c.offer, c.id AS comment_id, c.text, c.added, c.editedby, c.editedat, 
                                    ...
                                    WHERE c.offer = '.$id.' ORDER BY c.id '.$LIMIT) or sqlerr(__FILE__, __LINE__);
		$allrows = array();
...
if (isset($_POST['button']) && $_POST['button'] == 'Submit')
    ...
    sql_query ('INSERT INTO offers (offer_name, image, description, category, added, offered_by_user_id, link) VALUES 
                    ('.sqlesc($offer_name).', '.sqlesc($image).', '.sqlesc($body).', '.$category.', '.TIME_NOW.', '.$CURUSER['id'].',  '.sqlesc($link).');');
...
//=== start page
 $HTMLOUT .= '<table align="center" class="main" width="750px" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
        <td class="two" align="right">category:</td>
        <td align="left" class="two"><img border="0" src="pic/caticons/'.$CURUSER['categorie_icon'].'/'.htmlspecialchars($cat_image, ENT_QUOTES).'" alt="'.htmlspecialchars($cat_name, ENT_QUOTES).'" /></td>
    </tr>
    <tr>
        <td class="two" align="right">link:</td>
...
//===========================================================================================//
    ...
    if (!isset($id) || !is_valid_id($id))
      ...
      $res = sql_query('SELECT offer_name, offered_by_user_id FROM offers WHERE id ='.$id) or sqlerr(__FILE__,__LINE__);
      $arr = mysqli_fetch_assoc($res);
      if (!$arr)
...
require_once INCL_DIR.'bbcode_functions.php';
    ...
    $edit_res = sql_query('SELECT offer_name, image, description, category, offered_by_user_id, link FROM offers WHERE id ='.$id) or sqlerr(__FILE__,__LINE__);
...
if (isset($_POST['button']) && $_POST['button'] == 'Edit')
    ...
    sql_query ('UPDATE offers SET offer_name = '.sqlesc($offer_name).', image = '.sqlesc($image).', description = '.sqlesc($body).', 
                    category = '.sqlesc($category).', link = '.sqlesc($link).' WHERE id = '.$id);
...
require_once INCL_DIR.'pager_new.php';
    ...
    if (!isset($id) || !is_valid_id($id))
      ...
      $res = sql_query('SELECT offer_name FROM offers WHERE id = '.$id) or sqlerr(__FILE__,__LINE__);
      $arr = mysqli_fetch_assoc($res);
      if (!$arr)
 	...
 	if(isset($_POST['button']) && $_POST['button'] == 'Save')
      {
        ...
        @sql_query("INSERT INTO comments (user, offer, added, text, ori_text) VALUES (".$CURUSER['id'].",$id, ".TIME_NOW.", ".sqlesc($body).",".sqlesc($body).")");

        $newid = ((is_null($___mysqli_res = mysqli_insert_id($GLOBALS["___mysqli_ston"]))) ? false : $___mysqli_res);

        @sql_query('UPDATE offers SET comments = comments + 1 WHERE id = '.$id) or sqlerr(__FILE__,__LINE__);
...
$HTMLOUT .= $top_menu.'<form method="post" action="offers.php?action=add_comment">
	...
	<tr>
      ...
      $res = sql_query('SELECT c.offer, c.id AS comment_id, c.text, c.added, c.editedby, c.editedat, 
                                u.id, u.username, u.warned, u.suspended, u.enabled, u.donor, u.class, u.avatar, u.offensive_avatar, u.title, u.leechwarn, u.chatpost, u.pirate, u.king
                                FROM comments AS c
                                LEFT JOIN users AS u ON c.user = u.id 
                                WHERE offer = '.$id.' 
...
require_once INCL_DIR.'bbcode_functions.php';
      ...
      $res = sql_query('SELECT c.*, o.offer_name FROM comments AS c LEFT JOIN offers AS o ON c.offer = o.id WHERE c.id='.$comment_id) or sqlerr(__FILE__,__LINE__);
      $arr = mysqli_fetch_assoc($res);
      if (!$arr)
        stderr('Error', 'Invalid ID.');
...
echo stdhead('Edit comment to "'.htmlentities($arr['offer_name'], ENT_QUOTES ).'"', true, $stdhead).$HTMLOUT.stdfoot($stdfoot);
     break;
...
//===========================================================================================//
    ...
    case 'delete_comment':      
      ...
      $res = sql_query('SELECT user, offer FROM comments WHERE id='.$comment_id) or sqlerr(__FILE__,__LINE__);
      $arr = mysqli_fetch_assoc($res);
      if (!$arr)
        stderr('Error', 'Invalid ID.');
    ...
    if (!isset($_GET['do_it']))
        ...
        <a class="altlink" href="offers.php?action=delete_comment&amp;id='.$arr['offer'].'&amp;comment_id='.$comment_id.'&amp;do_it=666" >HERE</a>.');
        }
    else
        {
        sql_query('DELETE FROM comments WHERE id='.$comment_id);
        sql_query('UPDATE offers SET comments = comments - 1 WHERE id = '.$arr['offer']);
...
//===========================================================================================//
	...
	$ok_stuff = array('approved', 'pending', 'denied');
	//=== check it
    ...
    $message = sqlesc("Hi, \n An offer you made has been denied. \n\n  [url=".$INSTALLER09['baseurl']."/offers.php?action=offer_details&id=".$id."]".htmlspecialchars($arr_name['offer_name'], ENT_QUOTES)."[/url] was denied by ".$CURUSER['username'].". Please contact them to find out why.");
    sql_query('INSERT INTO messages (sender, receiver, added, msg, subject, saved, location) 
                VALUES(0, '.$arr_name['offered_by_user_id'].', '.TIME_NOW.', '.$message.', '.$subject.', \'yes\', 1)') or sqlerr(__FILE__, __LINE__);
    }   
        //=== ok, looks good :D let's set that status!
         sql_query('UPDATE offers SET status = '.sqlesc($change_it).' WHERE id = '.$id) or sqlerr(__FILE__, __LINE__);
...
function comment_table($rows)
{
  $count2 = '';
    ...
    ($row['id'] == $CURUSER['id'] || $CURUSER['class'] >= UC_STAFF ? '
     - [<a href="offers.php?action=edit_comment&amp;id='.$row['offer'].'&amp;comment_id='.$row['comment_id'].'">Edit</a>]' : '').($CURUSER['class'] >= UC_STAFF ? '
     - [<a href="offers.php?action=delete_comment&amp;id='.$row['offer'].'&amp;comment_id='.$row['comment_id'].'">Delete</a>]' : '').
     ($row['editedby'] && $CURUSER['class'] >= UC_STAFF ? '
     - [<a href="comment.php?action=vieworiginal&amp;cid='.$row['id'].'">View original</a>]' : '').' 
     - [<a href="report.php?type=Offer_Comment&amp;id_2='.$row['offer'].'&amp;id='.$row['comment_id'].'">Report</a>]';
