...
$status_drop_down = ($CURUSER['class'] < UC_STAFF ? '' : '<br /><form method="post" action="offers.php">
    ...
    <tr>
    <td class="colhead" align="center" colspan="2"><h1>'.htmlsafechars($arr['offer_name'], ENT_QUOTES).
...
//=== start page
 $HTMLOUT .= '<table align="center" class="main" width="750px" border="0" cellspacing="0" cellpadding="0">
   ...
   '.(isset($_POST['button']) && $_POST['button'] == 'Preview' ? '<br />
    ...
    $top_comment_stuff = $row['comment_id'].' by '.(isset($row['username']) ? print_user_stuff($row).($row['title'] !== '' ? ' [ '.htmlsafechars($row['title']).' ] ' :
    ...
    ($row['id'] == $CURUSER['id'] || $CURUSER['class'] >= UC_STAFF ? '
     ...
     - [<a href="offers.php?action=delete_comment&amp;id='.(int)$row['offer'].'&amp;comment_id='.(int)$row['comment_id'].'">Delete</a>]' : '').
