...
if ($game) {
    ...
    if ($start_ != 'yes') {
        ...
        if ($_POST["game"] == 'hit') $points = $aces = 0;
        ...
        foreach ($arr as $card_id) {
            ...
            if ($used_cards["points"] > 1) $points+= (int)$used_cards['points'];
            else $aces++;
        }
    }
    if ($_POST["game"] == 'hit') {
        if ($start_ == 'yes') {
            ...
            elseif ($arr['status'] == 'playing') stderr("Sorry", "You must finish your old game first.<form method='post' action='blackjack.php'><input type='hidden' name='game' value='hit' readonly='readonly' /><input type='hidden' name='continue' value='yes' readonly='readonly' /><input type='submit' value='Continue old game' /></form>");
        ...
        } elseif ((isset($_POST["start_"]) != 'yes' && isset($_POST['continue']) != 'yes') && !$gameover) {
