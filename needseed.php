...
$possible_actions = array('leechers','seeders');      
...
if ($needed == "leechers") {
    ...
    $res = sql_query("SELECT p.id, p.userid, p.torrent, u.username, u.uploaded, u.downloaded, t.name, t.seeders, t.leechers, t.category ".
    "FROM peers AS p ".
    "LEFT JOIN users AS u ON u.id=p.userid ".
    "LEFT JOIN torrents AS t ON t.id=p.torrent WHERE p.seeder = 'yes' ".
    "AND u.downloaded > '1024' AND u.added < $Dur ORDER BY u.uploaded / u.downloaded ASC LIMIT 20") or sqlerr(__FILE__, __LINE__);
    ...
    while ($arr = mysqli_fetch_assoc($res)) {
        ...
        $HTMLOUT .="<tr><td><a href='{$INSTALLER09['baseurl']}/userdetails.php?id=".(int)$arr['userid']."'>" .htmlspecialchars($arr['username'])."</a>&nbsp;(".member_ratio($arr['uploaded'], $arr['downloaded']).")</td><td><a href='{$INSTALLER09['baseurl']}/details.php?id=".(int)$arr['torrent']."' title='{$torrname}'>{$torrname}</a></td><td>{$cat}</td><td>{$peers}</td></tr>\n";
    }
    $HTMLOUT .="</table>\n";
    } else
