...
$newpage->check('tesu');
if (!mkglobal('wantusername:wantpassword:passagain:email'.($INSTALLER09['captcha_on'] ? ":captchaSelection:" : ":").'submitme:passhint:hintanswer')) stderr($lang['takesignup_user_error'], $lang['takesignup_form_data']);
...
if (empty($wantusername) || empty($wantpassword) || empty($email) || empty($passhint) || empty($hintanswer)) stderr($lang['takesignup_user_error'], $lang['takesignup_blank']);
...
//$ip = getip();
$ret = sql_query("INSERT INTO users (username, passhash, secret, editsecret, birthday, passhint, hintanswer, email, status, ".(!$arr[0] ? "class, " : "")."added, last_access, time_offset, dst_in_use, free_switch) VALUES (".implode(",", array_map("sqlesc", array(
    $wantusername,
    $wantpasshash,
    $secret,
    $editsecret,
