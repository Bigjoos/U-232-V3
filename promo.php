...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $do == "addpromo")	
	{
		...
		$q = sql_query("INSERT INTO promo (name,added,days_valid,max_users,link,creator,bonus_upload,bonus_invites,bonus_karma) VALUES (".implode(",", array_map("sqlesc",array($promoname,TIME_NOW,$days_valid,$max_users,$link,$CURUSER["id"],$bonus_upload,$bonus_invites,$bonus_karma))).") ") or sqlerr(__FILE__, __LINE__);
	...
	elseif($_SERVER["REQUEST_METHOD"] == "POST" && $do == "signup")
	{
          ...
          $passhash = make_passhash( $secret, md5($password) );
					...
					if($res)
					{
						...
						{$INSTALLER09['baseurl']}/confirm.php?id=$userid&secret=$sec
