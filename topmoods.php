...
if ($topmoods === false) {
   $res = sql_query('SELECT moods.*, users.mood, COUNT(users.mood) as moodcount '.
                    'FROM users LEFT JOIN moods ON (users.mood = moods.id) GROUP BY users.mood '.
                    'ORDER BY moodcount DESC, moods.id ASC') or sqlerr(__FILE__, __LINE__);

   while ($arr = mysqli_fetch_assoc($res)) {
      $topmoods  .= '<tr><td align="center">'.(int)$arr['moodcount'].'</td>
                 <td align="center">'.$arr['name'].' '.($arr['bonus'] == 1 ? '<a href="/mybonus.php" style="color:lime">(bonus)</a>' : '').'</td>
                 <td align="center"><img src="'.$INSTALLER09['pic_base_url'].'smilies/'.$arr['image'].'" alt="" /></td>
                 </tr>';
   }
