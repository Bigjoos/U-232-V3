...
$res1 = sql_query("SELECT 1 FROM thankyou WHERE torid=".sqlesc($id)." AND uid =" .$CURUSER["id"]. " ") or sqlerr(__FILE__, __LINE__);
...
sql_query("INSERT INTO thankyou (uid, torid, thank_date) VALUES (" .$CURUSER["id"]. ", ".sqlesc($id).", '" . TIME_NOW . "')") or sqlerr(__FILE__, __LINE__);
sql_query("INSERT INTO comments (user, torrent, added, text, ori_text) VALUES (" .$CURUSER["id"] . ",".sqlesc($id).", '" . TIME_NOW . "', " . sqlesc($text) ."," . sqlesc($text) . ")") or sqlerr(__FILE__, __LINE__);
...
$mc1->update_row(false, array('thanks' => $update['thanks'], 'comments' => $update['comments']));
