...
//if (isset($_GET['edit']) && ($_GET['user'] == $CURUSER['id']) && ($CURUSER['class'] >= UC_STAFF || ($arr['userid'] == $CURUSER['id']) && ($CURUSER['class'] >= UC_POWER_USER && $CURUSER['class'] <= UC_STAFF) && is_valid_id($_GET['edit'])))
...
if ( isset( $_GET['sent'] ) && ( $_GET['sent'] == "yes" ) ) {
    ...
    if (preg_match( $pattern, $text, $vars ) && $CURUSER["class"] >= UC_STAFF) {
	  ...
	  elseif(preg_match($private_pattern,$text,$vars)) {
        $to_user = 0;
        $p_res = sql_query(sprintf('SELECT id FROM users WHERE username = %s',sqlesc($vars[2]))) or sqlerr( __FILE__, __LINE__ );
        ...
        $a = mysqli_fetch_row(sql_query("SELECT userid, date FROM shoutbox ORDER by id DESC LIMIT 1")) or print("First shout or an error :)");
...
$res = sql_query( "SELECT s.id, s.userid, s.date, s.text, s.to_user, u.username, u.pirate, u.king, u.class, u.donor, u.warned, u.leechwarn, u.enabled, u.chatpost FROM shoutbox as s LEFT JOIN users as u ON s.userid=u.id ORDER BY s.id DESC LIMIT 30" ) or sqlerr( __FILE__, __LINE__ );
...
if($shout_pm_alert['pms'] > 0 && $gotpm == 0){
   ...
   foreach($shouts as $arr) {
	...
	if($arr['to_user'] == $CURUSER['id'] && $arr['to_user'] > 0)
        ...
        $reply = ($CURUSER['id'] != $arr['userid'] ? "<a href='javascript:window.top.SmileIT(\"[b][i]=>&nbsp;[color=#".get_user_class_color((int)$arr['class'])."]".htmlspecialchars($arr['username'])."[/color]&nbsp;-[/i][/b]\",\"shbox\",\"shbox_text\")'><img height='10' src='{$INSTALLER09['pic_base_url']}reply.gif' title='Reply' alt='Reply' style='border:none;' /></a>" : "" );
        $user_stuff = $arr;
