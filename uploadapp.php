...
if (isset($_POST["form"]) != "1") {
    ...
    $res = sql_query("INSERT INTO uploadapp(userid,applied,connectable,speed,offer,reason,sites,sitenames,scene,creating,seeding) VALUES({$app['userid']}, ".implode(",", array_map("sqlesc", array(TIME_NOW, $app['connectable'], $app['speed'], $app['offer'], $app['reason'], $app['sites'], $app['sitenames'], $app['scene'], $app['creating'], $app['seeding']))) .")") ;
