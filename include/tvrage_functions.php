...
function tvrage_format($tvrage_data,$tvrage_type) {
	$tvrage_display['show'] = array('showname'=>'<b>%s</b>','showlink'=>'%s','startdate'=>'Started: %s','ended'=>'Ended: %s','origin_country'=>'Country: %s','status'=>'Status: %s','classification'=>'Classification: %s','summary'=>'Summary:<br/> %s','runtime'=>'Runtime %s min','genres'=>'Genres: %s');
	$tvrage_display['episode'] = array('episode'=>'This week episode: %s "%s" on %s<br/>%s<br/>Summary: %s','nextepisode'=>'Next episode: %s "%s" on %s %s %s');
...
function tvrage(&$torrents) {
	...
	if(preg_match("/^(.*)S(\d+)(E(\d+))?/",$torrents['name'],$tmp))
		$tvrage = array('name'=>str_replace('.',' ',$tmp[1]),'season'=>(int)$tmp[2],'episode'=>(int)(isset($tmp[4]) ? $tmp[4] : 0));
	else
		$tvrage = array('name'=>str_replace('.',' ',$torrents['name']),'season'=>0,'episode'=>0);
	...
	if($force_update || ($tvrage_showinfo = $mc1->get_value($memkey)) === false) {
		...
		if(empty($torrents['newgenre']))
         ...
         $mc1->update_row(false, array('newgenre' => ucwords($tvrage_showinfo['genres'])));
         $mc1->commit_transaction(0);
		if(empty($torrents['poster'])) 
			$row_update[] = 'poster = '.sqlesc($tvrage_showinfo['image']);
		   //==The torrent cache
         $mc1->begin_transaction('torrent_details_'.$torrents['id']);
         $mc1->update_row(false, array('poster' => $tvrage_showinfo['image']));
         $mc1->commit_transaction(0);
      if(count($row_update))
