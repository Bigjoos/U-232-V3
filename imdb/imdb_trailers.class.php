...
//############################################################################
// $Id: imdb_trailers.class.php 253 2009-10-11 21:54:15Z izzy $
...
class imdb_trailers
{
    ...
    function getFlashCodeMovieMaze($url)
    {
        ...
        $mc = count($matches[0]);
        for ($i = 0; $i < $mc; ++$i) {
            if (strpos($matches[1][$i], "http://") === 0) $list[] = array(
                "url" => $matches[1][$i].".".$matches[2][$i],
                "format" => $matches[2][$i]
            );
            else $list[] = array(
                "url" => $this->moviemazeurl.$matches[1][$i].".".$matches[2][$i],
                "format" => $matches[2][$i]
            );
        }
        return $list;
    ...
    function getFlashCodeAllTrailers($url)
    {
        ...
        $mc = count($matches[1]);
        for ($i = 0; $i < $mc; ++$i) {
            if (strpos($matches[1][$i], "http://") === 0) $list[$i] = array(
                "url" => $matches[1][$i],
                "format" => "flv"
            );
            else $list[$i] = array(
                "url" => $this->alltrailersurl.$matches[1][$i],
                "format" => "flv"
            );
        }
        return $list;
    ...
    function getAllTrailers($mid)
    {
        ...
        foreach ($arraytrailers as $url) {
            unset($tl);
            $tmp = strtolower($url);
            if (strpos($tmp, "www.moviemaze.de") !== FALSE) $tl = $this->getFlashCodeMovieMaze($url);
            elseif (strpos($tmp, "alltrailers.net") !== FALSE) $tl = $this->getFlashCodeAllTrailers($url);
            elseif (strpos($url, "imdb.com/rg/VIDEO_TITLE/GALLERY") !== FALSE) $tl = $this->getImdbTrailers($url);
            elseif (strpos($tmp, "www.movieplayer.it") !== FALSE) $tl = $this->getMoviePlayerTrailers($url);
            elseif (strpos($tmp, "azmovietrailers.com") !== FALSE) $tl = $this->getAZMovieTrailers($url);
