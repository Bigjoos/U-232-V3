...
if (!empty($_GET["mid"])) {
    ...
    case "nm":
        header("Location: imdb_person.php?mid=".$_GET["mid"]);
        break;
...
foreach ($results as $res) {
    ...
    case "nm":
        ...
        echo " <TR><TD><a href='imdb_person.php?mid=".$res->imdbid()."'>".$res->name()."</a>$hint</TD>"."<TD><a href='http://".$search->imdbsite."/title/nm".$res->imdbid()."'>imdb page</a></TD></TR>\n";
        break;

    default:
        echo " <TR><TD><a href='imdb.php?mid=".$res->imdbid()."'>".$res->title()." (".$res->year().")</a></TD>"."<TD><a href='http://".$search->imdbsite."/title/tt".$res->imdbid()."'>imdb page</a></TD></TR>\n";
        break;
    }
}
echo "</TABLE>\n</BODY></HTML>";
