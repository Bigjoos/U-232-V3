...
class imdb_person extends imdb_base
{
    ...
    function birthname()
    {
        if (empty($this->birth_name)) {
            ...
            if (preg_match("/Birth Name<\/h5>\s*\n(.*?)\n/m", $this->page["Bio"], $match)) $this->birth_name = trim($match[1]);
        }
                ...
                for ($i = 0; $i < $mc; ++$i) {
                    ...
                    if (preg_match("/href\=\"\/OnThisDay\?day\=(\d{1,2}).{1,5}month\=(.*?)\".*\"\/MarriedInYear\?(\d{4})\">\d{4}<\/a>(.* href\=\"\/OnThisDay\?day=(\d{1,2}).{1,5}month=(.*?)\".*<\/a>\s*(\d{4})\)|)\s*\((.*?)\)(\s*(\d+) child|)/", $matches[2][$i], $match)) { // col#2: date, children
                        $tmp["from"] = array(
                            "day" => $match[1],
                            "month" => $match[2],
                            "mon" => $this->monthNo($match[2]) ,
                            "year" => $match[3]
                        );
                        $tmp["to"] = array(
                            "day" => $match[5],
                            "month" => $match[6],
                            "mon" => $this->monthNo($match[6]) ,
                            "year" => $match[7]
                        );
                        $tmp["comment"] = $match[8];
        ...
        if (empty($this->bio_bio)) {
            ...
            if (@preg_match_all('|<h5>Mini Biography</h5>\s*(.+)\s+.+\s+(.+)|', $this->page["Bio"], $matches)) {
                for ($i = 0; $i < count($matches[0]); ++$i) {
                    $bio_bio["desc"] = str_replace("href=\"/name/nm", "href=\"http://".$this->imdbsite."/name/nm", str_replace("href=\"/title/tt", "href=\"http://".$this->imdbsite."/title/tt", str_replace('/SearchBios', 'http://'.$this->imdbsite.'/SearchBios', $matches[1][$i])));
    ...
    function parparse($name, &$res)
    {
        ...
        if (preg_match_all("/<p>(.*?)<\/p>/ms", $block, $matches)) foreach ($matches[1] as $match) $res[] = str_replace('href="/name/nm', 'href="http://'.$this->imdbsite.'/name/nm', str_replace('href="/title/tt', 'href="http://'.$this->imdbsite.'/title/tt', $match));
    }
    //----------------------------------------------------------------[ Trivia ]---
...
class imdbpsearch extends imdbsearch
{
    ...
    function results($url = "")
    {
        ...
        if ($this->maxresults > 0) $maxresults = $this->maxresults;
        else $maxresults = 999999;
