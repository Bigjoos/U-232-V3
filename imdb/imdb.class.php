...
class imdb extends imdb_base
{
    ...
    function set_pagename($wt)
    {
        ...
        case "MovieConnections":
            $urlname = "/movieconnections";
            break;

        case "ExtReviews":
            $urlname = "/externalreviews";
            break;
        ...
        case "CompanyCredits":
            $urlname = "/companycredits";
            break;

        case "ParentalGuide":
            $urlname = "/parentalguide";
            break;

        case "OfficialSites":
            $urlname = "/officialsites";
            break;
    ...
    function runtime_all()
    {
        if ($this->main_runtime == "") {
            if ($this->page["Title"] == "") $this->openpage("Title");
            if (@preg_match("/Runtime:\<\/h5\>\s*<[^>]*>\s*(.*?)\n/m", $this->page["Title"], $match)) $this->main_runtime = $match[1];
        }
    ...
    function runtimes()
    {
        if (empty($this->movieruntimes)) {
            ...
            if (preg_match_all("/[\/ ]*((\D*?):|)([\d]+?) min( \((.*?)\)|)/", $this->main_runtime, $matches)) for ($i = 0; $i < count($matches[0]); ++$i) $this->movieruntimes[] = array(
                "time" => $matches[3][$i],
                "country" => $matches[2][$i],
                "comment" => $matches[5][$i]
            );
    ...
    */
    ...
    function comment()
    {
        if ($this->main_comment == "") {
            if ($this->page["Title"] == "") $this->openpage("Title");
            if (@preg_match("/\<div class\=\"comment\"(.*?)(\<b\>.*?)\<div class\=\"yn\"/ms", $this->page["Title"], $match)) $this->main_comment = preg_replace("/a href\=\"\//i", "a href=\"http://".$this->imdbsite."/", $match[2]);
    ...
    function comment_split()
    {
        if (empty($this->split_comment)) {
            ...
            if (@preg_match("/<b>(.*?)<\/b>, (.*)<br>.*?<a href=\"(.*)\">(.*?)<\/a>.*<p>(.*?)<\/p>/ms", $this->main_comment, $match)) $this->split_comment = array(
                "title" => $match[1],
                "date" => $match[2],
                "author" => array(
                    "url" => $match[3],
    ...
    function keywords()
    {
        if (empty($this->main_keywords)) {
            if ($this->page["Title"] == "") $this->openpage("Title");
            if (preg_match_all("/\<a href\=\"\/keyword\/[\w\-]+\/\"\>(.*?)\<\/a\>/", $this->page["Title"], $matches)) $this->main_keywords = $matches[1];
        }
    ...
    function languages()
    {
        if (empty($this->langs)) {
            if ($this->page["Title"] == "") $this->openpage("Title");
            if (preg_match_all('!<a href="/language/.*?">\s*(.*?)\s*<!m', $this->page["Title"], $matches)) $this->langs = $matches[1];
        }
        return $this->langs;
    }
    //--------------------------------------------------------------[ Genre(s) ]---
    ...
    function genres()
    {
        if (empty($this->moviegenres)) {
            if ($this->page["Title"] == "") $this->openpage("Title");
            if (preg_match_all("@<a href\=\"/genre/[\w\-]+\"\>(.*?)\</a>@", $this->page["Title"], $matches)) $this->moviegenres = $matches[1];
        }
    ...
    function colors()
    {
        if (empty($this->moviecolors)) {
            if ($this->page["Title"] == "") $this->openpage("Title");
            if (preg_match_all("|/search/title\?colors=.*?>\s*(.*?)<|", $this->page["Title"], $matches)) $this->moviecolors = $matches[1];
        }
        ...
        if (empty($this->main_creator)) {
            ...
            if (@preg_match("/Creator:\<\/h5\>\s*\n(.*?)(<\/div|<a class=\"tn15more)/ms", $this->page["Title"], $match)) {
                if (preg_match_all('|/name/nm(\d{7}).*?>(.*?)<|ims', $match[1], $matches)) {
                    for ($i = 0; $i < count($matches[0]); ++$i) $this->main_creator[] = array(
                        'name' => $matches[2][$i],
                        'imdb' => $matches[1][$i]
                    );
                }
    ...
    */
    ...
    function country()
    {
        if (empty($this->countries)) {
            if ($this->page["Title"] == "") $this->openpage("Title");
            $this->countries = array();
            if (preg_match_all("/\/country\/\w+\"\>(.*?)<\/a/m", $this->page["Title"], $matches)) for ($i = 0; $i < count($matches[0]); ++$i) $this->countries[$i] = $matches[1][$i];
        }
    ...
    function sound()
    {
        if (empty($this->sound)) {
            if ($this->page["Title"] == "") $this->openpage("Title");
            if (preg_match_all("|/search/title\?sound_mixes=.*?>\s*(.*?)<|", $this->page["Title"], $matches)) $this->sound = $matches[1];
        }
        return $this->sound;
    }
    //-------------------------------------------------------[ MPAA / PG / FSK ]---
    ...
    function mpaa_reason()
    {
        if (empty($this->mpaa_justification)) {
            if ($this->page["Title"] == "") $this->openpage("Title");
            if (preg_match("/href=\"\/mpaa\"\>.*?<\/h5\>\s*<[^>]*>\s*(.*?)\s*<\/div/", $this->page["Title"], $match)) $this->mpaa_justification = trim($match[1]);
        }
    ...
    function plot()
    {
        if (empty($this->plot_plot)) {
            ...
            if (preg_match_all("/p class=\"plotpar\">(.*?)<\/p>/", str_replace("\n", " ", $this->page["Plot"]) , $matches)) for ($i = 0; $i < count($matches[0]); ++$i) $this->plot_plot[$i] = preg_replace('/<a href=\"\/SearchPlotWriters/i', '<a href="http://'.$this->imdbsite.'/SearchPlotWriters/', $matches[1][$i]);
        }
    ...
    function plot_split()
    {
        if (empty($this->split_plot)) {
            ...
            for ($i = 0; $i < count($this->plot_plot); ++$i) {
                if (preg_match("/(.*?)<i>.*<a href=\"(.*?)\">(.*?)<\/a>/", $this->plot_plot[$i], $match)) $this->split_plot[] = array(
                    "plot" => $match[1],
                    "author" => array(
                        "name" => $match[3],
                        "url" => $match[2]
    ...
    function synopsis()
    {
        if (empty($this->synopsis_wiki)) {
            ...
            if (preg_match('|<div id="swiki\.2\.1">(.*?)</div>|ims', $this->page["Synopsis"], $match)) $this->synopsis_wiki = trim($match[1]);
        }
    ...
    function taglines()
    {
        if (empty($this->taglines)) {
            ...
            if (preg_match_all("/<p>(.*?)<\/p><hr/", $this->page["Taglines"], $matches)) $this->taglines = $matches[1];
        }
    ...
    function get_table_rows_cast($html, $table_start, $class = "nm")
    {
        ...
        if ($row_s == 0) return FALSE;
        $endtable = strpos($html, "</table>", $row_s);
        if (preg_match_all("/<tr.*?(<td class=\"$class\".*?)<\/tr>/", substr($html, $row_s, $endtable - $row_s) , $matches)) return $matches[1];
        return array();
    }
    //------------------------------------------------------[ Helper: RowCells ]---
    ...
    public function cast()
    {
        ...
        for ($i = 0; $i < count($cast_rows); $i++) {
            ...
            if (strpos($dir["thumb"], '._V1')) $dir["photo"] = preg_replace('|(.*._V1)\..+\.(.*)|is', '$1.$2', $dir["thumb"]);
    ...
    function crazy_credits()
    {
        if (empty($this->crazy_credits)) {
            ...
            if (preg_match_all("/<li><tt>(.*?)<\/tt><\/li>/", $cred, $matches)) $this->crazy_credits = $matches[1];
        }
    ...
    function goofs()
    {
        if (empty($this->goofs)) {
            ...
            if (preg_match_all("/<li>.*?<b>(.*?)<\/b>(.*?)<br>\s*<br><\/li>/ims", $goofs, $matches)) {
                $gc = count($matches[1]);
                for ($i = 0; $i < $gc; ++$i) $this->goofs[] = array(
                    "type" => $matches[1][$i],
                    "content" => $matches[2][$i]
                );
            }
    ...
    function quotes()
    {
        if (empty($this->moviequotes)) {
            ...
            if (preg_match_all('|<a name="qt.*?</a>\s*(.*?)<p class="linksoda"|ims', str_replace("\n", " ", $this->page["Quotes"]) , $matches)) foreach ($matches[1] as $match) $this->moviequotes[] = str_replace('href="/name/', 'href="http://'.$this->imdbsite.'/name/', $match);
        }
    ...
    function trailers()
    {
        if (empty($this->trailers)) {
            ...
            if (!empty($tag_s)) { // trailers on the IMDB site itself
                ...
                if (preg_match_all('|<a href="{0,1}(/video/screenplay/vi\d*/)|ims', $trail, $matches)) for ($i = 0; $i < count($matches[0]); ++$i) {
        ...
        if (empty($this->soundtracks)) {
            ...
            if (preg_match('|<p class="disclaimer">(.*?)</script|ims', $this->page["Soundtrack"], $match)) {
                if (preg_match_all("/<li>(.*?)<\/b><br>(.*?)<br>(.*?)<br>.*?<\/li>/", str_replace("\n", " ", $match[1]) , $matches)) {
                    $mc = count($matches[0]);
                    for ($i = 0; $i < $mc; ++$i) $this->soundtracks[] = array(
                        "soundtrack" => $matches[1][$i],
                        "credits" => array(
    ...
    function keywords_all()
    {
        if (empty($this->all_keywords)) {
            ...
            if (preg_match_all('|<li><b class="keyword">\s*<a href\="/keyword\/[\w\-]+/">(.*?)</a>|', $this->page["Keywords"], $matches)) $this->all_keywords = $matches[1];
        }
