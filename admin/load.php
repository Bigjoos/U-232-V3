...
function uptime() {
	...
	$fd = fopen ($filename, "r");
	if ($fd===false) {
		$res="Could not retrieve uptime";
	} else {
		$uptime = fgets($fd, 64);
		fclose ($fd);

		$mults = array(4=>"month",7=>"week",24=>"day",60=>"hour",1=>"minute");
		$n = 2419200;
		$periods = array();
		$shown=false;
		...
		while (list($k,$v)=each($mults)) {
			$nmbr = floor($uptime / $n); $uptime -= ($nmbr*$n);
			$n = $n / $k;
			if ($nmbr) {
				if ($shown) $res.=", ";
		...
		if (!$shown) { $res.="less than one minute"; }
	}
	return $res;
}
...
function loadavg($return_all=false) {
	...
	$fd = fopen ($filename, "r");
	if ($fd===false) {
		$res="Could not retrieve load average";
	} else {
		$loadavg = fgets($fd, 64);
		fclose ($fd);
