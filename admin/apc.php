...
//  this work!
defaults('ADMIN_USERNAME', 'apc_stats'); // Admin Username
defaults('ADMIN_PASSWORD', '09source_coder'); // Admin Password - CHANGE THIS TO ENABLE!!!
...
$MY_SELF = "{$INSTALLER09['baseurl']}/staffpanel.php?tool=apc"."&amp;SCOPE=".$MYREQUEST['SCOPE']."&amp;SORT1=".$MYREQUEST['SORT1']."&amp;SORT2=".$MYREQUEST['SORT2']."&amp;COUNT=".$MYREQUEST['COUNT'];
$MY_SELF_WO_SORT = "{$INSTALLER09['baseurl']}/staffpanel.php?tool=apc"."&amp;SCOPE=".$MYREQUEST['SCOPE']."&amp;COUNT=".$MYREQUEST['COUNT'];
...
if (isset($MYREQUEST['IMG'])) {
    ...
    if ($MYREQUEST['IMG'] == 3) $image = imagecreate(2 * $size + 150, $size + 10);
...
EOB;
    if (!empty($_SERVER['SERVER_NAME'])) echo "<tr class='tr-0'><td class='td-0'>APC Host</td><td>{$_SERVER['SERVER_NAME']} $host</td></tr>\n";
...
EOB;
    ...
    if ($mem['num_seg'] > 1 || $mem['num_seg'] == 1 && count($mem['block_lists'][0]) > 1) $mem_note = "Memory Usage<br /><font size='-2'>(multiple slices indicate fragments)</font>";
    else $mem_note = "Memory Usage";
    echo <<< EOB
		</tbody></table>
		</div>
...
EOB;
    ...
    if (graphics_avail()) {
        ...
        echo <<<EOB
			<img alt="" $size src="{$INSTALLER09['baseurl']}/staffpanel.php?tool=apc&amp;IMG=3&amp;$time">
EOB;
...
EOB;
    ...
    echo <<<EOB
		...
		<select name='SCOPE'>
EOB;
    echo "<option value='A", $MYREQUEST['SCOPE'] == 'A' ? " selected=selected" : "", "'>Active</option>", "<option value='D", $MYREQUEST['SCOPE'] == 'D' ? " selected=selected" : "", "'>Deleted</option>", "</select>", ", Sorting:<select name=SORT1>", "<option value='H", $MYREQUEST['SORT1'] == 'H' ? " selected=selected" : "", "'>Hits</option>", "<option value='Z", $MYREQUEST['SORT1'] == 'Z' ? " selected=selected" : "", "'>Size</option>", "<option value='S", $MYREQUEST['SORT1'] == 'S' ? " selected=selected" : "", "'>$fieldheading</option>", "<option value='A", $MYREQUEST['SORT1'] == 'A' ? " selected=selected" : "", "'>Last accessed</option>", "<option value='M", $MYREQUEST['SORT1'] == 'M' ? " selected=selected" : "", "'>Last modified</option>", "<option value='C", $MYREQUEST['SORT1'] == 'C' ? " selected=selected" : "", "'>Created at</option>", "<option value='D", $MYREQUEST['SORT1'] == 'D' ? " selected=selected" : "", "'>Deleted at</option>";
    if ($fieldname == 'info') echo "<option value=D", $MYREQUEST['SORT1'] == 'T' ? " selected=selected" : "", ">Timeout</option>";
    echo '</select>', '<select name=SORT2>', '<option value=D', $MYREQUEST['SORT2'] == 'D' ? ' selected=selected' : '', '>DESC</option>', '<option value=A', $MYREQUEST['SORT2'] == 'A' ? ' selected=selected' : '', '>ASC</option>', '</select>', '<select name=COUNT onChange="form.submit()">', '<option value=10 ', $MYREQUEST['COUNT'] == '10' ? ' selected=selected' : '', '>Top 10</option>', '<option value=20 ', $MYREQUEST['COUNT'] == '20' ? ' selected=selected' : '', '>Top 20</option>', '<option value=50 ', $MYREQUEST['COUNT'] == '50' ? ' selected=selected' : '', '>Top 50</option>', '<option value=100', $MYREQUEST['COUNT'] == '100' ? ' selected=selected' : '', '>Top 100</option>', '<option value=150', $MYREQUEST['COUNT'] == '150' ? ' selected=selected' : '', '>Top 150</option>', '<option value=200', $MYREQUEST['COUNT'] == '200' ? ' selected=selected' : '', '>Top 200</option>', '<option value=500', $MYREQUEST['COUNT'] == '500' ? ' selected=selected' : '', '>Top 500</option>', '<option value=0  ', $MYREQUEST['COUNT'] == '0' ? ' selected=selected' : '', '>All</option>', '</select>', '&nbsp; Search: <input name=SEARCH value="', $MYREQUEST['SEARCH'], '" type=text size=25/>', '&nbsp;<input type=submit value="GO!">', '</form></div>', '<div class="info"><table cellspacing="0"><tbody>', '<tr>', '<th>', sortheader('S', $fieldheading, "&amp;OB=".$MYREQUEST['OB']) , '</th>', '<th>', sortheader('H', 'Hits', "&amp;OB=".$MYREQUEST['OB']) , '</th>', '<th>', sortheader('Z', 'Size', "&amp;OB=".$MYREQUEST['OB']) , '</th>', '<th>', sortheader('A', 'Last accessed', "&amp;OB=".$MYREQUEST['OB']) , '</th>', '<th>', sortheader('M', 'Last modified', "&amp;OB=".$MYREQUEST['OB']) , '</th>', '<th>', sortheader('C', 'Created at', "&amp;OB=".$MYREQUEST['OB']) , '</th>';
    if ($fieldname == 'info') {
        $cols+= 2;
    ...
    if ($list) {
        ...
        foreach ($list as $k => $entry) {
            if (!$MYREQUEST['SEARCH'] || preg_match('/'.$MYREQUEST['SEARCH'].'/i', $entry[$fieldname]) != 0) {
                ...
                echo '<td class="td-last center">', $entry['deletion_time'] ? date(DATE_FORMAT, $entry['deletion_time']) : '-', '</td>', '</tr>';
                $i++;
...
case OB_SYS_CACHE_DIR:
    ...
    echo <<<EOB
		...
		<select name=SCOPE>
EOB;
    echo "<option value=A", $MYREQUEST['SCOPE'] == 'A' ? " selected=selected" : "", ">Active</option>", "<option value=D", $MYREQUEST['SCOPE'] == 'D' ? " selected=selected" : "", ">Deleted</option>", "</select>", ", Sorting:<select name=SORT1>", "<option value=H", $MYREQUEST['SORT1'] == 'H' ? " selected=selected" : "", ">Total Hits</option>", "<option value=Z", $MYREQUEST['SORT1'] == 'Z' ? " selected=selected" : "", ">Total Size</option>", "<option value=T", $MYREQUEST['SORT1'] == 'T' ? " selected=selected" : "", ">Number of Files</option>", "<option value=S", $MYREQUEST['SORT1'] == 'S' ? " selected=selected" : "", ">Directory Name</option>", "<option value=A", $MYREQUEST['SORT1'] == 'A' ? " selected=selected" : "", ">Avg. Size</option>", "<option value=C", $MYREQUEST['SORT1'] == 'C' ? " selected=selected" : "", ">Avg. Hits</option>", '</select>', '<select name=SORT2>', '<option value=D', $MYREQUEST['SORT2'] == 'D' ? ' selected=selected' : '', '>DESC</option>', '<option value=A', $MYREQUEST['SORT2'] == 'A' ? ' selected=selected' : '', '>ASC</option>', '</select>', '<select name=COUNT onChange="form.submit()">', '<option value=10 ', $MYREQUEST['COUNT'] == '10' ? ' selected=selected' : '', '>Top 10</option>', '<option value=20 ', $MYREQUEST['COUNT'] == '20' ? ' selected=selected' : '', '>Top 20</option>', '<option value=50 ', $MYREQUEST['COUNT'] == '50' ? ' selected=selected' : '', '>Top 50</option>', '<option value=100', $MYREQUEST['COUNT'] == '100' ? ' selected=selected' : '', '>Top 100</option>', '<option value=150', $MYREQUEST['COUNT'] == '150' ? ' selected=selected' : '', '>Top 150</option>', '<option value=200', $MYREQUEST['COUNT'] == '200' ? ' selected=selected' : '', '>Top 200</option>', '<option value=500', $MYREQUEST['COUNT'] == '500' ? ' selected=selected' : '', '>Top 500</option>', '<option value=0  ', $MYREQUEST['COUNT'] == '0' ? ' selected=selected' : '', '>All</option>', '</select>', ", Group By Dir Level:<select name='AGGR'>", "<option value='' selected='selected'>None</option>";
    for ($i = 1; $i < 10; $i++) echo "<option value='$i", $MYREQUEST['AGGR'] == $i ? " selected" : "", "'>$i</option>";
