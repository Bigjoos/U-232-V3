...
loggedinorreturn();
   ...
   $posted_action = (isset($_POST['action']) ? $_POST['action'] : (isset($_GET['action']) ? $_GET['action'] : ''));
	//=== add all possible actions here and check them to be sure they are ok
   $valid_actions = array('flush_torrents', 'staff_notes', 'watched_user');
		...
		switch ($action)
		{
			...
			$id = isset($_POST['id']) ? intval($_POST['id']) : 0;
				...
				if ($id == $CURUSER['id'])
				{
					...
					sql_query('UPDATE snatched SET seeder=\'no\' WHERE userid = '.$CURUSER['id']);
						//=== flush dem torrents!!! \o/
						sql_query('DELETE FROM peers WHERE userid = '.$CURUSER['id']);
				...
				{
				//=== it's a staff... 
				$res_get_info = sql_query('SELECT username FROM users WHERE id='.$id);
    				$user_get_info = mysqli_fetch_assoc($res_get_info);
					//=== catch any missed snatched stuff thingies to stop ghost leechers from getting peers (if the peers they have drop off)
					sql_query('UPDATE snatched SET seeder=\'no\' WHERE userid = '.$id);
						//=== flush dem torrents!!! \o/
			...
			$staff_notes_res = sql_query('SELECT staff_notes, class, username FROM users WHERE id='.$id) or sqlerr(__FILE__, __LINE__);
			...
			sql_query('UPDATE users SET staff_notes = '.sqlesc($posted_notes).' WHERE id ='.$id) or sqlerr(__FILE__, __LINE__);
         $mc1->begin_transaction('user'.$id);
         $mc1->update_row(false, array('staff_notes' => $posted_notes));
         $mc1->commit_transaction($INSTALLER09['expires']['user_cache']);
			//=== add it to the log
			write_log('<b>'.$CURUSER['username'].'</b> edited member <a href="userdetails.php?id='.$id.'" title="go to '.$staff_notes_arr['username'].(substr($staff_notes_arr['username'], -1) == 's' ? '\'' : '\'s').' staff notes"><b>'.$staff_notes_arr['username'].(substr($staff_notes_arr['username'], -1) == 's' ? '\'' : '\'s').'</b></a> staff notes. Changes made:<br />Was:<br />'.$staff_notes_arr['staff_notes'].'<br />is now:<br />'.htmlspecialchars($_POST['new_staff_note']).'');
			}
			...
			//=== make sure they are staff, not editing their own and playing nice :P
			$watched_res = sql_query('SELECT watched_user, watched_user_reason, class, username FROM users WHERE id='.$id) or sqlerr(__FILE__, __LINE__);
			...
			if ($id !== $CURUSER['id'] || $CURUSER['class'] < $watched_arr['class'])
			{
				...
				if (isset($_POST['add_to_watched_users']) && $_POST['add_to_watched_users'] == 'yes' && $watched_arr['watched_user'] == 0)
				{
					...
					sql_query('UPDATE users SET watched_user = '.TIME_NOW.' WHERE id = '.$id) or sqlerr(__FILE__, __LINE__);
               ...
               $mc1->commit_transaction($INSTALLER09['expires']['user_cache']);
					//=== add it to the log
					write_log('<b>'.$CURUSER['username'].'</b> added member <a href="userdetails.php?id='.$id.'" title="go to '.$watched_arr['username'].(substr($watched_arr['username'], -1) == 's' ? '\'' : '\'s').' page">'.$watched_arr['username'].'</a> to watched users.');
				}
				if (isset($_POST['add_to_watched_users']) && $_POST['add_to_watched_users'] == 'no' && $watched_arr['watched_user'] > 0)
				{
					...
					sql_query('UPDATE users SET watched_user = 0 WHERE id = '.$id) or sqlerr(__FILE__, __LINE__);
               $mc1->begin_transaction('MyUser_'.$id);
               ...
               $mc1->commit_transaction($INSTALLER09['expires']['user_cache']);
					//=== add it to the log
					write_log('<b>'.$CURUSER['username'].'</b> removed member <a href="userdetails.php?id='.$id.'" title="go to '.$watched_arr['username'].(substr($watched_arr['username'], -1) == 's' ? '\'' : '\'s').' page">'.$watched_arr['username'].'</a> from watched users. <br />'.$watched_arr['username'].' had been on the list since '.get_date( $watched_arr['watched_user'],'').'.', $CURUSER['id']);
				}
				//=== only change if different	
				if($_POST['watched_reason'] !== $watched_arr['watched_user_reason'])
				{
				//=== edit watched users text
				sql_query('UPDATE users SET watched_user_reason = '.sqlesc($posted).' WHERE id = '.$id) or sqlerr(__FILE__, __LINE__);
            $mc1->begin_transaction('user'.$id);
            $mc1->update_row(false, array('watched_user_reason' => $posted));
            $mc1->commit_transaction($INSTALLER09['expires']['user_cache']);
				//=== add it to the log
				write_log('<b>'.$CURUSER['username'].'</b> changed watched user text for: <a href="userdetails.php?id='.$id.'" title="go to '.$watched_arr['username'].(substr($watched_arr['username'], -1) == 's' ? '\'' : '\'s').' page">'.$watched_arr['username'].'</a>  Changes made:<br />Text was:<br />'.$watched_arr['watched_user_reason'].'<br />Is now:<br />'.htmlspecialchars($_POST['watched_reason']).'');
				}
			}
