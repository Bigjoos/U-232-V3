...
if ($name) $pager = "sub_name=".$name."&amp;";
$searchby = (isset($_GET["searchby"]) ? $_GET["searchby"] : "");
if ($searchby) $pager.= "searchby=".$searchby."&amp;";
$lang = (isset($_GET["lang"]) ? $_GET["lang"] : "all");
if ($lang) $pager.= "lang=".$lang."&amp;";
$fps = (isset($_GET["fps"]) ? $_GET["fps"] : "");
if ($fps) $pager.= "fps=".$fls."&amp;";
$format = (isset($_GET["format"]) ? $_GET["format"] : "");
if ($format) $pager.= "format=".$format."&amp;";
$cds = (isset($_GET["cds"]) ? 0 + $_GET["cds"] : "");
if ($cds) $pager.= "cds=".$cds."&amp;";
$offset = (isset($_GET["offset"]) ? 0 + $_GET["offset"] : "");
if ($searchby == "name") $name = str_replace(array(
    ".",
    "/",
    "\"",
    "!",
...
<form action=" " method="get" >
  <table width="40%" cellpadding="10" border="1" cellspacing="0" style="border-collapse:collapse" align="center">
    <tr>
      ...
      <td align="left" colspan="7" nowrap="nowrap"><input type="text" name="sub_name" value="<?php
echo ($name ? $name : "") ?>" size="80" />
        &nbsp;by&nbsp;
        <select name="searchby">
          <option value="name" <?php
echo ($searchby == "name" ? "selected=\"selected\"" : "") ?>>Name</option>
          <option value="imdb" <?php
echo ($searchby == "imdb" ? "selected=\"selected\"" : "") ?>>IMDb id</option>
        </select></td>
    </tr>
    <tr>
      <td nowrap="nowrap" align="center">Subtitle format</td>
      <td><?php
echo (build_menu("format", $format_menu, $format)) ?></td>
      <td nowrap="nowrap" align="center">Cds</td>
      <td><?php
echo (build_menu("cds", $cds_menu, $cds)) ?></td>
      <td nowrap="nowrap" align="center">Language</td>
      <td><?php
echo (build_menu("lang", $lang_menu, $lang)) ?></td>
      <td nowrap="nowrap" align="center">FPS</td>
      <td><?php
echo (build_menu("fps", $fps_menu, $fps)) ?></td>
    </tr>
