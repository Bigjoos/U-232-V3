...
{
    $optional = "";
    if (isset($lang)) $optional.= "/sublanguageid-".$lang;
    if (isset($cds)) $optional.= "/subsumcd-".$cds;
    if (isset($format)) $optional.= "/subformat-".$format;
    if (isset($offset) && $offset > 0) $optional.= "/offset-".$offset;
    if (isset($searchby) && $searchby == "name") $search = "/moviename-".urlencode($name);
    if (isset($searchby) && $searchby == "imdb") {
        if (!ereg("[0-9]{7}", $name, $imdbid)) die("Can't find imdb id");
        else $search = "/imdbid-".$imdbid[0];
    }
...
function get_details($array)
{
    ...
    foreach ($array as $key => $value) {
        foreach ($value as $key2 => $value2) $details[$key] = $value2;
    }
    return $details;
}
...
{
    $pager = "";
    if ($itemsfound > 40) $links = ($itemsfound / 40);
    if (intval($links) != $links) $links = intval($links) + 1;
    if ($links > 1) {
        $pager = "";
        for ($i = 0; $i < $links; $i++) {
            if ($i % 15 == 0) $pager.= "<br/><br/>";
            if (($i * 40) == 1000) break;
...
function build_result($array, $pager)
{
    ...
    //print the content
    if (count($result) == 0) print ("<div align=\"center\"><h2>No result found</h2></div>");
    else {
?>
...
?>
            ...
            echo ($movie["iso639"]) ?>.gif" width="18" height="12" border="0" alt="<?php
            echo ($movie["language"]) ?>" title="<?php
            echo ($movie["language"]) ?>" /></td>
				<td align="left" colspan="2" width="100%"><a href="<?php
            echo ($base.$movie["detail"]) ?>" target="_blank">
				  <?php
            echo ($movie["movie"]) ?>
				  </a>
				  <?php
            ...
            echo ($base.$movie["download"]) ?>" target="blank"><img src="imgs/icon-download.gif" width="12" height="12" border="0" alt=" " title="download" /></a></td>
				<td nowrap="nowrap" align="center"><?php
            echo (str_replace(" ", "<br/>", $movie["subadddate"])) ?></td>
				<td nowrap="nowrap" align="center"><?php
            echo ($movie["files"]) ?></td>
				<td nowrap="nowrap" align="center"><?php
            echo ($movie["format"]) ?></td>
				<td nowrap="nowrap" align="center"><?php
            echo ($movie["user"] == "" ? "Unknown" : $movie["user"]) ?></td>
			  </tr>
			  <?php
        } ?>
			</table>
