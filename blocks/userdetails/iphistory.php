...
//== iphistory
   if ($user['paranoia'] < 2 || $CURUSER['id'] == $id ) 
   {
    ...
    if(($iphistory = $mc1->get_value('ip_history_'.$id)) === false) {
        ...
        $HTMLOUT .= "<tr><td class='rowhead'>{$lang['userdetails_address']}</td><td align='left'>{$addr}{$iphistory['use']}&nbsp;(<a class='altlink' href='staffpanel.php?tool=iphistory&amp;action=iphistory&amp;id=".(int)$user['id']."'><b>History</b></a>)&nbsp;(<a class='altlink' href='staffpanel.php?tool=iphistory&amp;action=iplist&amp;id=".(int)$user['id']."'><b>List</b></a>)</td></tr>\n";
        if ($CURUSER["class"] >= UC_STAFF && $iphistory['ips'] > 0)
        $HTMLOUT .="<tr><td class='rowhead'>IP History</td><td align='left'>This user has earlier used <b><a href='{$INSTALLER09['baseurl']}/staffpanel.php?tool=iphistory&amp;action=iphistory&amp;id=".(int)$user['id']."'>{$iphistory['ips']} different IP addresses</a></b></td></tr>\n";
        }
//==end
// End Class
