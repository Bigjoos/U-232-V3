...
//== Users friends list
    ...
    $fr = sql_query("SELECT f.friendid as uid, f.userid AS userid, u.last_access, u.id, u.ip, u.avatar, u.username, u.class, u.donor, u.title, u.warned, u.enabled, u.chatpost, u.leechwarn, u.pirate, u.king, u.downloaded, u.uploaded FROM friends AS f LEFT JOIN users as u ON f.friendid = u.id WHERE userid=".sqlesc($id)." ORDER BY username ASC LIMIT 100") or  sqlerr(__file__, __line__);
    ...
    $user_friends .= "<tr><td class='one' style='padding: 0px; border: none' width='40px'>".$avatar."</td><td class='one'>".format_username($user_stuff)."<br />".($CURUSER['class'] >= UC_STAFF ? "".htmlsafechars($a['ip'])."" : "")."</td><td class='one' style='padding: 1px' align='center'>".mksize($a['uploaded'])."</td><td class='one' style='padding: 1px' align='center'>".mksize($a['downloaded'])."</td><td class='one' style='padding: 1px' align='center'>".member_ratio($a['uploaded'], $a['downloaded'])."</td><td class='one' style='padding: 1px' align='center'>".$status."</td></tr>\n";
    }
    $user_friends .= "</table>";
