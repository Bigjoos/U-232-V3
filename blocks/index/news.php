<?php
    ...
    if($news === false ) {
    $res = sql_query("SELECT ".$prefix.".id, ".$prefix.".userid, ".$prefix.".added, ".$prefix.".title, ".$prefix.".body, ".$prefix.".sticky, u.username FROM news AS ".$prefix." LEFT JOIN users AS u ON u.id = ".$prefix.".userid WHERE ".$prefix.".added + ( 3600 *24 *45 ) > ".TIME_NOW." ORDER BY sticky, ".$prefix.".added DESC LIMIT 10") or sqlerr(__FILE__, __LINE__);
    ...
    <a href='staffpanel.php?tool=news&amp;mode=edit&amp;newsid={$array['id']}'>
    <img src='{$INSTALLER09['pic_base_url']}button_edit2.gif' border='0' alt=\"Edit news\"  title=\"Edit news\" /></a>&nbsp;
    <a href='staffpanel.php?tool=news&amp;mode=delete&amp;newsid={$array['id']}&amp;h={$hash}'>
    ...
    <img border=\"0\" src='pic/plus.png' id=\"pica{$array['id']}\" alt=\"Show/Hide\" />&nbsp;" .get_date( $array['added'],'DATE') . "&nbsp;-&nbsp;" ."".htmlspecialchars($array['title'])."</a>&nbsp;-&nbsp;Added by <a href='./userdetails.php?id=".(int)$array['userid']."'><b>".htmlspecialchars($array['username'])."</b></a>
    {$button}
    </div></div>";
    $HTMLOUT .="
